(function (angular) {
    ...
    angular
            ...
            return function (problems, domains) {
                if (problems != undefined && problems != false && problems != true) {
                    return problems.filter(function (problem) {
                        ...
                        if (problem.account.domain != null) {
                            if (domains.indexOf(problem.account.domain.name) != -1) {
                                return true;
                            }
                        }
                        return false;
        ...
        .controller('MyBusinessProblemController', MyBusinessProblemController);
    MyBusinessProblemController.$inject = ['$scope', '$state', 'ProblemService', '$timeout', '$window', '$rootScope', '$mdDialog', '$stateParams'];
    function MyBusinessProblemController($scope, $state, ProblemService, $timeout, $window, $rootScope, $mdDialog, $stateParams) {
        ...
        $scope.getBusinessProblem = function () {
            ProblemService.getUserAllProblems($stateParams.id, function (response) {
        ...
        ProblemService.getUserAllProblems($stateParams.id, function (problemlist) {
        ...
        ProblemService.getRecentProblems($stateParams.id, function (response) {
        ...
        $scope.filterByDate = function () {
            if ($scope.fromDate != null && $scope.toDate != null) {
                ProblemService.getProblemsByDate($scope.fromDate, $scope.toDate, function (response) {
                    $scope.problems = response;
                })
            }
        }
        ...
        $scope.getMyProblem = function () {
            ProblemService.getUserAllProblems($stateParams.id, function (problems) {
        ...
        $scope.problemKeywordFilter = function (filter) {
            return function (problem) {
                if (filter == "") {
                    ...
                    if (problemId.indexOf(filter) >= 0) {
                        return true;
                    }
                    else if (problemStatement.indexOf(filter) >= 0) {
                        return true;
                    }
                    else if (businessImpact.indexOf(filter) >= 0) {
                        return true;
                    }
        ...
        $scope.getProblem = function () {
            ...
            if ($scope.myId3) {
                ProblemService.getUserProblems($stateParams.id, function (problems) {
                    $scope.problems = problems;
                });
...
//        $scope.getMyBusinessProblem = function () {
//            ProblemService.getUserAllProblems($stateParams.id, function (response) {
//                $scope.problems = response;
//                $scope.checkuserlikes();
//                $scope.loadMore();
...
//            ProblemService.likeProblem(problem, function (response) {
//                likedProblem.likes.push(problem);
//                $scope.problems[index].userlikes = [];
//                $scope.problems[index].userlikes.push(loggedInUser);
//                ProblemService.getRecentProblems(loggedInUser, function (responseRecent) {
//                    $scope.recentProblems = responseRecent;
//                });
//            })
